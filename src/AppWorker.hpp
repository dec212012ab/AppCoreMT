...
public:
	 ...
	 * 
	 * @param immortal : If True, then the worker cannot be ended manually.
	 */
	AppWorker(bool immortal=false):worker_id(AppWorker::worker_id_gen++),immortal(immortal){
		std::stringstream ss;
		ss<<worker_id;
	 ...
	 * @param immortal : If True, then the worker cannot be ended manually.
	 */
	AppWorker(std::string name, bool immortal=false):worker_id(AppWorker::worker_id_gen++),immortal(immortal),worker_name(name){};
	~AppWorker()=default;
	...
	/**
	 * @brief Get the value of the immortal flag for the worker
	 * 
	 * @return true if the worker was set to be immortal
	 * @return false if the worker was not set to be immortal
	 */
	bool getImmortal(){
		return immortal;
	};
...
private:
	...
	bool immortal = false; ///< Whether or not this thread is allowed to be stopped while the program is active and not exiting
